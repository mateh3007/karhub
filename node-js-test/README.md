...
- **E2E** (`npm run test:e2e`): sobe a aplicação real contra o Postgres do `docker-compose`, cobrindo o fluxo completo (registro → login → criar peça → ver em `/restock/priorities`), autorização por papel, e isolamento entre empresas (uma empresa não lê, edita ou deleta dados de outra).
